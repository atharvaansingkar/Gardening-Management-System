...
<div class="container-xxl py-5">
        ...
        <div class="row g-4">
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<img class='img-fluid' src='/img/flowers.png' alt='Icon'>";
                    echo "</div>";

                    echo "<form action='new5_user.php' method='POST'>";
                    echo "<input type='hidden' name='flower_name' value='" . $row['flower_name'] . "'>";
                    echo "<input type='submit' name='submit' value='" . $row['flower_name'] . "'></input>";
                    echo "</form>";
    ...
    <div class="container mx-auto p-4">
        <form method="post" action="process_form5.php">
            <div class="mb-4">
            ...
            <div class="mb-4">
                <label for="flowername" class="block text-gray-600 font-bold">Flower Name:</label>
                <select id="flowername" name="flowername" class="rounded-lg border px-3 py-2 w-full" required>
                    <option value="" disabled selected>Select a Flower</option>
                    <?php
...
$query = "SELECT flower_name FROM flowers";
$result = mysqli_query($db_connection, $query);

if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<option value='" . $row['flower_name'] . "'>" . $row['flower_name'] . "</option>";
    }
} else {
    echo "<option value='' disabled>No flowers found</option>";
}
